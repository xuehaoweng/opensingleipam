  ...
  <div class="main-container">
    ...
    >
      ...
      <n-space style="margin-bottom: 5px">
        ...
        <n-popconfirm @positive-click="recycle_many_func">
          <template #trigger>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function get_new() {
        ...
        if (subnet_info.value['next_page_url']) {
          ...
          get({
            ...
            subnet_info.value.result_list = subnet_info.value.result_list.concat(
              next_res['results']
            )
            subnet_info.value.next_page_url = next_res['next']
            subnet_info.value.maxhosts =
              Number(subnet_info.value.maxhosts) + Number(next_res['results'].length)
      ...
      function handleScroll(event) {
        ...
        if (
          event.srcElement.scrollTop + document.body.clientHeight >=
      ...
      function export_ipam_detail() {
        ...
        for (let i = 0; i < detail_list.length; i++) {
          export_str =
            ...
            ',' +
            (detail_list[i]['lastOnlineTime'] === null
              ? '暂无'
        ...
        get({
          ...
          if (resp) {
            ...
            subnet_info.value = {
              ...
              freehosts: res.subnet_used.freehosts.toString(),
              Used_percent:
                res.subnet_used['已分配已使用_percent'] + res.subnet_used['未分配已使用_percent'],
              freehosts_percent: res.subnet_used.freehosts_percent.toString(),
              maxhosts: res.subnet_used.maxhosts.toString(),
              count: parseInt(res.subnet_used.maxhosts),
