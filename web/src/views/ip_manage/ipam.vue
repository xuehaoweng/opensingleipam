  ...
  <div class="main-container">
    <div style="width: 25%; padding: 10px; border-top: 1px solid #000; float: left">
      ...
      <div style="padding-top: 20px; border-top: 1px solid; border-right: 1px solid">
        ...
        <n-button size="tiny" type="info" @click="open_export" style="float: left">导出网段
        </n-button>
        <n-button size="tiny" type="info" style="float: right" @click="create_subnet()">新增网段
        </n-button>
        <!--        </n-space>-->
        <!--        左侧树形结构展示网段信息-->
        <n-tree :data="tree_data" virtual-scroll :node-props="nodeProps" :default-expand-all="false"
          :show-irrelevant-nodes="false" :render-switcher-icon="renderSwitcherIcon" style="height: 600px"
          :pattern="pattern" block-line />
    ...
    <!--    详情-饼图-文字-展示图-->
    <div v-show="detail_show" style="width: 70%; height: 100%; padding: 10px; float: right; border-top: 1px solid #000">
      ...
      <n-scrollbar trigger="none" style="max-height: 500px" @scroll="handleScroll">
        <div>
          <n-button style="width: 160px;padding:5px 0;margin: 0 10px 10px 0px;" :id="ip_info.address"
    ...
    <ModalDialog ref="modalDialog" title="网段信息" :style="{ height: '768px', width: '1000px' }">
      <template #content>
        <n-button type="info" size="small" style="float: right" @click="export_ipam">导出EXCEL
        </n-button>
        <n-data-table :loading="subnet_loading" :data="subnet_list.slice((subnet_page - 1) * subnet_pageSize, subnet_page * subnet_pageSize)
          " :columns="subnet_tableColumns" :row-key="rowKey" />
        <div class="flex justify-center">
          <n-pagination v-model:page="subnet_page" :page-count="subnet_pageCount" show-size-picker
            :page-sizes="subnet_pageSizes" />
        </div>
      </template>
    </ModalDialog>

    <ModalDialog ref="table_modalDialog" title="网段详情表格形式" :style="{ height: '768px', width: '1000px' }">
      <template #content>
        <n-button type="info" size="small" style="float: right" @click="export_ipam_detail">导出EXCEL
        </n-button>
        <n-data-table :data="detail_list.slice((detail_page - 1) * detail_pageSize, detail_page * detail_pageSize)
          " :columns="detail_tableColumns" :row-key="detail_rowKey" />
        <div class="flex justify-center">
          <n-pagination v-model:page="detail_page" v-model:page-size="detail_pageSize" :page-count="detail_pageCount"
            show-size-picker :page-sizes="detail_pageSizes" />
        </div>
      </template>
    </ModalDialog>
    ...
    <ModalDialog ref="dispatch_many_modalDialog" title="批量地址分配" :style="{ height: '650px', width: '800px' }"
    ...
    <n-modal v-model:show="add_root_show" preset="dialog" title="新增根">
      <div>
        <n-form :model="add_root_form" label-placement="top" label-width="auto">
          <n-form-item label="子网Subnet">
            <n-input v-model:value="add_root_form.add_subnet" placeholder="8.8.0.0/16严格CIDR格式" />
          </n-form-item>
          ...
          <n-form-item label="父节点master">
            <n-input v-model:value="add_root_form.master_subnet_id.split('-')[0]" disabled placeholder="ROOT" />
          </n-form-item>
        </n-form>
      </div>
      <template #action>
    ...
    <n-modal v-model:show="add_subnet_show" preset="dialog" title="新增子网">
      <div>
        <n-form :model="add_subnet_form" label-placement="top" label-width="auto">
          <n-form-item label="子网Subnet">
            <n-input v-model:value="add_subnet_form.add_subnet" placeholder="8.8.0.0/16严格CIDR格式" />
          </n-form-item>
          <n-form-item label="网段描述\nDescription">
          ...
          <n-form-item label="父节点master">
            <n-input v-model:value="add_subnet_form.master_subnet_id.split('-')[0]" disabled placeholder="ROOT" />
          </n-form-item>
        </n-form>
      </div>
      <template #action>
    ...
    <n-modal v-model:show="change_desc_show" preset="dialog" title="修改网段描述">
      <n-form :model="change_desc_form" label-placement="top" label-width="auto">
        <n-form-item label="子网">
          <n-input v-model:value="change_desc_form.subnet" placeholder="8.8.0.0/16严格CIDR格式" disabled />
        </n-form-item>
        <n-form-item label="描述">
          <n-input v-model:value="change_desc_form.description" placeholder="description"
