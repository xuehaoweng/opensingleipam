<template>
  <div class="main-container">
    ...
    <ModalDialog ref="modalDialog" title="新建定时任务" @confirm="CreateSubmitConfirm"
      ...
      <template #content>
        <DataForm ref="itemDataFormRef" preset="grid-two-item" :options="itemFormOptions" :form-config="{
          labelWidth: 120,
          labelAlign: 'left'
        }" />
      </template>
    ...
    <ModalDialog ref="EditmodalDialog" title="编辑celery任务" @confirm="EditFormConfirm"
      ...
      <template #content>
        <DataForm ref="EdititemDataFormRef" :options="EdititemFormOptions" :form-config="{
          labelWidth: 80,
        }" />
      </template>
    </ModalDialog>
